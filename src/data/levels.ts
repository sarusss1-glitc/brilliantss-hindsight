...
// A piece is REMOVED when its final undo exits the board (undoPath had one segment).
// WIN when all pieces are removed.
// ============================================================

import type { Level } from "../types";
import { type LegacyPiece, pieceFromLegacy } from "./pieceFromLegacy";

type RawLevel = Omit<Level, "pieces"> & { pieces: LegacyPiece[] };

// ============================================================
// UNDO TARGET HELPER (for reference — implement in gameLogic.ts)
//
// Current direction: piece.undoPath[0] (see gameLogic.getUndoTarget).
//
...
export const LEVELS: Level[] = RAW_LEVELS.map((level) => ({
  ...level,
  pieces: level.pieces.map(pieceFromLegacy),
}));
